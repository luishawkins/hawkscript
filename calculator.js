...
var calculator = (function() {
    ...
    var lexer = (function() {
        var lexer = ({
            ...
            rules: [/^(?:\s+)/, /^(?:[0-9]+(\.[0-9]+)?\b)/, /^(?:por\b)/, /^(?:partelo\b)/, /^(?:tequito\b)/, /^(?:tedoy\b)/, /^(?:\^)/, /^(?:!)/, /^(?:%)/, /^(?:dentroizq\b)/, /^(?:dentroder\b)/, /^(?:PI\b)/, /^(?:E\b)/, /^(?:$)/, /^(?:.)/],
...
function separador(cadenaADividir) {
    var regularExpre = /;|\n/
    var arrayDeCadenas = cadenaADividir.split(regularExpre);
    arrayDeCadenas = arrayDeCadenas.filter(texto => texto.length > 0);
    var arrayDeResultado = [];
    for (var i = 0; i < arrayDeCadenas.length; i++) {
        //console.log(arrayDeCadenas[i]);
        if (arrayDeCadenas[i] != "") {
            arrayDeResultado[i] = evalu(arrayDeCadenas[i]);
        }
